...
export default function AppPage() {
  ...
  const handleSubmit = async () => {
    if (!file) {
      toast({ title: "No file selected", description: "Please choose a document to process." })
      return
    }

    setIsProcessing(true)
    setSteps((s) => s.map((st) => ({ ...st, status: st.key === "upload" ? "running" : "idle" })))

    try {
      await wait(600)
      setSteps((s) =>
        s.map((st) =>
          st.key === "upload" ? { ...st, status: "done" } : st.key === "analyze" ? { ...st, status: "running" } : st,
        ),
      )
      await wait(900)
      setSteps((s) =>
        s.map((st) =>
          st.key === "analyze" ? { ...st, status: "done" } : st.key === "extract" ? { ...st, status: "running" } : st,
        ),
      )
      ...
      await wait(700)
      setSteps((s) => s.map((st) => (st.key === "extract" ? { ...st, status: "done" } : st)))
      setDocData(data)
      toast({ title: "Document extracted", description: `Found ${data.Tasks.length} tasks.` })
    } catch (e: any) {
      setSteps((s) => s.map((st) => (st.status === "running" ? { ...st, status: "error" } : st)))
      toast({ title: "Processing failed", description: e?.message ?? "Unknown error" })
    } finally {
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-1">
            ...
            <CardContent className="space-y-4">
              <FileUploader value={file} onChange={setFile} accept={[".pdf", ".docx", ".md", ".txt"]} />
              <div className="flex items-center gap-2">
                <Checkbox id="math-mode" checked={mathMode} onCheckedChange={(v) => setMathMode(v === true)} />
                ...
                <ol className="space-y-2">
                  {steps.map((step) => (
                    <li key={step.key} className="flex items-center justify-between text-sm">
                      ...
                      <Badge
                        variant={
                          step.status === "done" ? "secondary" : step.status === "error" ? "destructive" : "outline"
                        }
                      >
            ...
            <CardHeader>
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <CardDescription>
                    {hasData ? "Review the title, description, and tasks" : "No data yet"}
                  </CardDescription>
                </div>
                <Button
                  variant="outline"
              ...
              {hasData ? (
                <>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-sm text-gray-700 whitespace-pre-line">{docData!.Description}</p>
                  </div>
                  <Separator />
                  <div className="space-y-3">
                    ...
                    <TaskList
                      tasks={docData!.Tasks}
                      onChange={(updated) => setDocData((prev) => (prev ? { ...prev, Tasks: updated } : prev))}
                    />
                  </div>
                </>
              ) : (
                <div className="text-sm text-gray-500">Extract a document to see its details here.</div>
              )}
            </CardContent>
          </Card>
        </div>
...
async function mockExtract(file: File, mathMode: boolean): Promise<ExtractedDocDataDTO> {
